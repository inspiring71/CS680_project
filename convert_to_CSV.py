...
def get_commit_info(revisions):
    ...
    for i in revisions:
        if entropy==0:
            ...
            all_files = 0
            for commit_file in revisions[i]['files']:
                file_directories = commit_file.split('/')
                file_directories.pop()
                subsystems.append('/'.join(file_directories))
    ...
    return max_files_addition, max_files_deletion, max_files_delta, max_files_size, commit_sbj, commit_msg, commit_id, entropy, files_touched, AVG_NUR, hotness_percentage, subsystems_touched
...
for i in tqdm(range(0,record_num,batch_size), position=0, leave=True):
    ...
    # print(collection_batch.count())
    for review in tqdm(collection_batch, position=0, leave=True,total=(batch_size)):
        # try:
        #     diff = json.dumps(get_git_diff(review['id'])) 
        #     # print(diff)
        # except:
        #     continue
        max_files_addition, max_files_deletion, max_files_delta, max_files_size, commit_sbj, commit_msg, commit_id, entropy, files_touched, AVG_NUR, hotness_percentage, subsystems_touched = get_commit_info(review['revisions'])
        ...
        if not (review['project'] in project_start):
           ...
           project_reviews[review['project']] += 1
        if (review['status']=="ABANDONED" or review['status']=="MERGED" ):
            decision = True
        else:
            decision = False
        ...
        tmp = {
            ...
            'entropy': entropy,
            'subsystems':subsystems_touched,
            'files_touched' : files_touched,
            'has_topic':'topic' in review,
            'topic': review['topic'] if 'topic' in review else False,
            ...
            'has_assignee':True if 'assignee' in review else False,
            'owner_is_submitter': True if ('submitter' in review and review['submitter']['_account_id'] == review['owner']['_account_id']) else False,
            'project':review['project'],
            'project_age': (creation - project_start[review['project']]),
            ...
            'average_NUR' : AVG_NUR,
            'owener' : review['owner']['_account_id'],
            ...
            'commit_time':datetime.datetime.strptime(review['created'][:-4], '%Y-%m-%d %H:%M:%S.%f').hour
            # 'diff':diff
        }
        ...
        # break
        i+=1
    # extracted_reviews.to_pickle('computed_'+str(int((i+1)/batch_size))+'.pkl')
